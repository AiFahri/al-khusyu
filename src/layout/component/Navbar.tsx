...
export default function Navbar() {
  ...
  // Close dropdown when clicking outside of the education menu area (desktop/tablet/mobile)
  useEffect(() => {
    function isTrueDesktop() {
      if (typeof window === "undefined") return false;
      return window.matchMedia("(pointer: fine) and (hover: hover)").matches;
    }

    if (!isTrueDesktop()) return; 

    const handlePointerDown = (event: Event) => {
      if (!eduRef.current) return;
      const target = event.target as Node | null;
      const clickedOutside = !!target && !eduRef.current.contains(target);
      if (clickedOutside) {
        if (eduOpen) {
          setEduOpen(false);
        }
  ...
  useEffect(() => {
    // console.log("[Navbar] eduOpen:", eduOpen);
  }, [eduOpen]);
          ...
          <div className="relative" ref={eduRef}>
            <button
              className={`flex items-center gap-1 outline-none ${
                currentPath.startsWith("/pendidikan") ? "text-[#FBFED0]" : ""
              }`}
              aria-haspopup="menu"
            ...
            {eduOpen && (
              ...
              >
                <div className="grid grid-cols-1 sm:grid-cols-2 lg:grid-cols-4 gap-x-12 gap-y-4 p-6 lg:p-8">
                  <div>
                    <div className="text-[#63817E] font-semibold mb-3">
                      Pendidikan Formal
                    </div>
                    <Link
                      to="/pendidikan/tk"
                      className="block py-2 hover:text-[#63817E]"
                      onClick={() => {
                        setEduOpen(false);
                      }}
                    >
                  ...
                  <div>
                    ...
                    <Link
                      to="/pendidikan/sanggar"
                      className="block py-2 hover:text-[#63817E]"
                      onClick={() => {
                        setEduOpen(false);
                      }}
                    >
                      Sanggar Al-Qur'an Al-Khusyu'
                    </Link>
                  </div>
              ...
              <div>
                <button
                  ...
                  className={`w-full flex items-center justify-between py-3 px-4 rounded-lg font-semibold transition-colors ${
                    currentPath.startsWith("/pendidikan")
                      ? "bg-[#63817E] text-white"
                      : "text-gray-700 hover:bg-gray-100"
                  }`}
                >
                ...
                {eduOpen && (
                  <div className="mt-2 pl-4 space-y-2">
                    <div className="text-sm font-medium text-[#63817E] mb-2">
                      Pendidikan Formal
                    </div>
                    <Link
                      to="/pendidikan/tk"
                      className="block py-2 pl-4 text-gray-600 hover:text-[#63817E] hover:bg-gray-50 rounded"
                    >
                      RA Al-Khusyu'
                    </Link>
                    <Link
                    ...
                    <div className="text-sm font-medium text-[#63817E] mb-2 mt-4">
                      Pesantren & Tahfidz
                    </div>
                    <Link
                      to="/pendidikan/tahfidz"
